...
# Uses Goddard stage to evaluate severity of emphysema on imaging and GOLD stage to evaluate severity of airflow obstruction
# Nick Mark, 2017
...
copd <- select(copd, gold_stage, goddard_score, new_gold, new_goddard, colNames, sample, copd_anydefinition,
               cd45, cd3, cd4, cd8, nkt, #gdt, nk, b, mac, pmn,  # basic cell types
               cd8ifng, th1, th17, treg, gdtil17, gdtifng,      # cytokine profiles
              #cd8_pd1, cd4_pd1,                                # pd-1 expression
...
minus_gold <- select(copd, -new_gold, -new_goddard, 
